...
class Uploads
{
    ...
    function __construct($options=null) {
        $this->options = array(
            ...
            'upload_dir' => dirname($_SERVER['SCRIPT_FILENAME']).'/uploads/files/',
            'upload_url' => $this->getFullUrl().'/uploads/files/',
            'param_name' => 'files',
    ...
    protected function get_file_object($file_name) {
        ...
        if (is_file($file_path) && $file_name[0] !== '.') {
            ...
            $pedazo=substr(trim($file->name), -7);
            $type=substr($pedazo,strpos($pedazo,".")+1).".png";
            foreach($this->options['image_versions'] as $version => $options) {
                if (is_file($options['upload_dir'].$file_name)) {
                    ...
                    {//echo $options['upload_dir']."no_tocar/".rawurlencode($type);
                        if (file_exists($options['upload_dir']."no_tocar/".rawurlencode($type)))
                             $file->{$version.'_url'} = $options['upload_url']."/no_tocar/".rawurlencode($type);
        ...
        if (!$error && $file->name) {
            ...
            if ($file_size === $file->size) {
                ...
                foreach($this->options['image_versions'] as $version => $options) {
                    ...
                    {$new=substr($type,strpos($type, "/")+1).".png";
                        if (file_exists($options['upload_dir']."no_tocar/".rawurlencode($new)))
                             $file->{$version.'_url'} = $options['upload_url']."/no_tocar/".rawurlencode($new);
